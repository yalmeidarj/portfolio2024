...
export default function SortButton({ categories }: { categories: string[] }) {
    ...
    const [selectedCategory, setSelectedCategory] = useState<string>( techSkills.all);
        ...
        <div className="w-full">
            ...
            <div className="flex mx-auto flex-wrap justify gap-1 mb-2"> 
                ...
                {categories.map(category => (
                    <button
                        key={category}
                        className={`px-2 py-1 text-xs font-medium rounded-md shadow-sm ${selectedCategory === category ? 'bg-custom-text-accent-light dark:bg-custom-title dark:text-custom-text-dark text-custom-text-dark' : 'bg-custom-title dark:bg-custom-text-dark dark:text-custom-text-light  dark:light-bg-gradient-dark text-custom-midBackground-light'}`} 
